...
public abstract class RTNode {
    ...
    protected Rectangle[] data; // 相当于条目，实际上会多出的一个用于结点分裂
    ...
    public RTNode(RTree rtree, RTNode parent, int level) {
        ...
        data = new Rectangle[capacity + 1];// 多出的一个用于结点分裂
        usedSpace = 0;
    }

    /**
     * 节点中的条目，其中会多出的一个用于结点分裂
     *
     * @return 节点中的条目
     */
    ...
    protected void deleteData(int i) {
        if (data[i + 1] != null) // 如果为中间节点（非尾节点），采用拷贝数组的方式链接条目
        {
    ...
    protected int[][] quadraticSplit(Rectangle rectangle) {
        ...
        data[usedSpace] = rectangle; // 先添加进去
        ...
        while (rem > 0) {
            ...
            if (i2 == c) {
                ...
                if (areaDiff1.compareTo(areaDiff2) < 0)// 先比较面积增量
                {
                    group1[i1++] = sel;
                } else if (areaDiff1.compareTo(areaDiff2) > 0) {
                    group2[i2++] = sel;
                } else if (mbr1.getArea().compareTo(mbr2.getArea()) < 0)// 再比较自身面积
                {
                    group1[i1++] = sel;
